...
# aunque podemos ser un poco más exigente con el a que usaremos (más
# pequeño).
...
# 
# Esta pregunta es sobre el parámetro µ de la población desconocida,
...
cat("\n")
cat("H0: µ =", µ, "[1/pixeles]\n")
cat("HA: µ <>", µ, "[1/pixeles]\n")
cat("siendo µ la media de las mediciones ADL en imágenes de riñones\n\n")
...
z <- (media.muestra - µ) / (desvest.muestra / sqrt(tamaño.muestra))
p.valor <- pnorm(z)
...
ztest1 <- z.test(muestra, mu = µ, sd = desvest.muestra,
                 alternative = "two.sided", conf.level = 1 - a)
...
# Notemos que se incluye el intervalo de 100(1 - a)% confianza
cat("\n")
...
cat("H0: µ =", µ, "[s]\n")
cat("HA: µ >", µ, "[s]\n")
cat("siendo µ la media del tiempo de permanencia de llamas en los\n")
...
t <- (media.muestra - µ) / (desvest.muestra / sqrt(tamaño.muestra))
...
ttest1 <- t.test(muestra, mu = µ,
                 alternative = "greater", conf.level = 1 - a)
...
# suelo en cualquier punto, la pregunta es sobre su parámetro µ. 
# Las hipótesis serían:
# H0: µ = 3  [%]
# HA: µ <> 3 [%]
...
cat("H0: µ =", µ, "[%]\n")
cat("HA: µ <>", µ, "[%]\n")
cat("siendo µ la media del porcentaje de materia orgánica del\n")
...
ztest2 <- z.test(muestra, mu = µ, sd = desvest.muestra,
                 alternative = "two.sided", conf.level = 1 - a)
z <- ztest2[["statistic"]]
...
ttest2 <- t.test(muestra, mu = µ, conf.level = 1 - a)
...
# Esto ocurriría si el peso medio µ de todas las tabletas (población)
...
cat("H0: µ =", µ, "[g]\n")
cat("HA: µ <>", µ, "[g]\n")
cat("siendo µ el peso medio de todas las tabletas fabricadas.\n\n")
...
ztest3 <- z.test(muestra, mu = µ, sd = desvest.muestra,
                 alternative = "two.sided", conf.level = 1 - a)
z <- ztest3[["statistic"]]
...
ttest3 <- t.test(muestra, mu = µ, conf.level = 1 - a)
...
# se está cumpliendo, el tiempo de activación de medio µ de todos los
...
cat("H0: µ =", µ, "[s]\n")
cat("HA: µ >", µ, "[s]\n")
cat("siendo µ el tiempo de activación de medio requerido por todos\n")
