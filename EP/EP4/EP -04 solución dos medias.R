...
# Si µ es la diferencia media del contenido total de minerales en los 
...
cat("-----------------------------------------------------------------\n")
cat("µ hipotético de las diferencias:", µ0, "g\n")
...
# Vemos que el p.valor es menor que el a definido para la prueba, por lo
...
ttest1 <- t.test(posd, lact, paired = TRUE,
                 mu = 25, alternative = "greater", conf.level = 1 - a
)
...
# no es correcto para nuestra hipótesis µ > 25.
# Así, o cambiamos la hipótesis a µ < 25 o cambiamos el orden de los
...
# estos suplementos. Llamemos µ_linaza al peso medio que alcanzan (todos)
# los pollos alimentados con suplementos basados en linaza, y llamemos
# µ_habas a (todos) los pollos alimentados con suplementos basados en
...
# H0: µ_habas y µ_linaza no son diferentes (µ_habas = µ_linaza)
# H1: µ_habas y µ_linaza sí son diferentes (µ_habas <> µ_linaza)
# o en términos de fiderencia de estas medias:
# H0: µ_habas - µ_linaza = 0  
# H1: µ_habas - µ_linaza <> 0
#
µ0 <- 0
...
cat("-----------------------------------------------------------\n")
cat("Diferencia de µ's hipotética:", µ0, "g\n")
cat("Diferencia de µ's observada:", dif.medias, "g\n")
...
ttest3 <- t.test(weight ~ feed, data = datos, mu = µ0, conf.level = 1 - a)
...
# El p-valor es bastante menor que el a definido, por lo que hay una
