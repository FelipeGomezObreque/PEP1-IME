...
p.10a44.esp <- .55
p.45omás.esp <- .70
d.esp <- p.10a44.esp - p.45omás.esp
...
A <- Za * sqrt((r + 1) * P * Q)
B <- Zß * sqrt(r * p.10a44.esp  * (1 - p.10a44.esp) +
...
a <- 0.05
z.a.medio <- qnorm(1 - a / 2)

p.prima.num <- p.gorro + (z.a.medio^2 / (2 * n))
p.prima.den <- 1 + (z.a.medio^2 / n)
p.prima <- p.prima.num / p.prima.den

s.prima.num.term1 <- (p.gorro * (1 - p.gorro)) / n
s.prima.num.term2 <- z.a.medio^2 / (4 * n^2)
s.prima.num <- sqrt(s.prima.num.term1 + s.prima.num.term2)
s.prima.den <- 1 + (z.a.medio^2 / n)
s.prima <- s.prima.num / s.prima.den

ic.low <- p.prima - z.a.medio * s.prima
ic.upp <- p.prima + z.a.medio * s.prima

cat("\n")
cat("Nivel de significación:", a, "\n")
cat("Intervalo con 100·(1-a)% de confianza: ")
cat("[", ic.low, ", ", ic.upp, "]", "\n", sep = "")
# stop()
...
cat("\n")
cat("Diferencia en proporciones observadas:", d.obs, "\n")
# stop()
...
err.est.d <- sqrt(var.est.1 + var.est.2)

a <- 0.05
z.a.medio <- qnorm(1 - a / 2)

if(d.obs > 0) {
  ic.low <- d.obs - z.a.medio * err.est.d
  ic.upp <- d.obs + z.a.medio * err.est.d
} else {
  ic.low <- d.obs + z.a.medio * err.est.d
  ic.upp <- d.obs - z.a.medio * err.est.d
}
...
# equivalente, usando una prueba ?². Si no se especifica el parámetro p,
